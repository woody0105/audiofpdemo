...
from dejavu.database_handler.es import Es
...
class Dejavu_Es:
    ...
    def __load_fingerprinted_audio_hashes(self) -> None:
        ...
        self.songhashes_set = set()  # to know which ones we've computed before
        for song in self.songs:
            song_hash = song['_source'][FIELD_FILE_SHA1]
            self.songhashes_set.add(song_hash)
    ...
    def fingerprint_directory(self, path: str, extensions: str, nprocesses: int = None) -> None:
        ...
        # Send off our tasks
        iterator = pool.imap_unordered(Dejavu_Es._fingerprint_worker, worker_input)
    ...
    def fingerprint_file(self, file_path: str, song_name: str = None) -> None:
        ...
        if song_hash in self.songhashes_set:
            ...
            song_name, hashes, file_hash, title, artist= Dejavu_Es._fingerprint_worker(
                (file_path,
                self.limit,)
            )
            sid = self.db.insert_song(song_name, title, artist, file_hash, len(hashes))
    ...
    def align_matches(self, matches: List[Tuple[int, int]], dedup_hashes: Dict[str, int], queried_hashes: int,
        ...
        for song_id, offset, _ in songs_matches[0:topn]:  # consider topn elements in the result
            ...
            song_title = song[FIELD_SONGTITLE]
            artist = song[FIELD_ARTIST]
            song_hashes = song[FIELD_TOTAL_HASHES]
            nseconds = round(float(offset) / DEFAULT_FS * DEFAULT_WINDOW_SIZE * DEFAULT_OVERLAP_RATIO, 5)
            hashes_matched = dedup_hashes[song_id]
